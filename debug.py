# Jetsonでないデバイス上でのデバッグ用のスクリプト：COCO Datasetをダウンロードし，人がどのグリッドにいるかで画像を振り分ける
...
def download_coco_images(output_dir="images", person_images=100, no_person_images=100):
    """
    COCOデータセットから画像をダウンロード（人がいる画像といない画像をランダムに選択）

    Args:
        output_dir (str): 出力ディレクトリ
        person_images (int): 人がいる画像の枚数（実際は混在）
        no_person_images (int): 人がいない画像の枚数（実際は混在）
    """
    print("COCOデータセットから画像をダウンロードしています...")
    ...
    # ランダムな画像ID（実際のCOCO val2017画像IDの一部）
    with open("coco_val2017_image_ids.txt", "r") as f:
        # 画像IDのリストを読み込む
        sample_image_ids = [line.strip() for line in f.readlines()]

    # リストをシャッフル
    random.shuffle(sample_image_ids)

    total_images = person_images + no_person_images
    selected_ids = sample_image_ids[:total_images]

    downloaded_count = 0
    failed_count = 0

    for i, img_id in enumerate(selected_ids):
        file_name = f"coco_{img_id}.jpg"
        img_url = base_url + f"{img_id}.jpg"
        save_path = os.path.join(output_dir, file_name)

        # 既に存在する場合はスキップ
        if os.path.exists(save_path):
            print(f"スキップ ({i + 1}/{len(selected_ids)}): {file_name} (既に存在)")
            continue

        try:
            print(f"ダウンロード中 ({i + 1}/{len(selected_ids)}): {file_name}")
            ...
            failed_count += 1
            print(f"ダウンロード失敗: {file_name} - {str(e)}")
            continue
    ...
    print("注意: ダウンロードした画像には人がいる画像といない画像が混在しています。")
    print("      main()関数を実行してYOLOで検出・振り分けを行ってください。")
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "download":
        # 画像をダウンロード
        print("COCOデータセットから画像をダウンロードします...")
        download_coco_images(
            output_dir="images", person_images=100, no_person_images=100
        )
        print(
            "\nダウンロード完了！次に以下のコマンドで推論・振り分けを実行してください:"
        )
